...
const App: React.FC = () => {
  ...
  const handleExportData = () => {
    ...
    a.href = url;
    a.download = `daily-harmony-cloud-backup-${format(new Date(), 'yyyy-MM-dd')}.json`;
  ...
  const handleImportData = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (confirm('클라우드 서버에 백업 데이터를 동기화하시겠습니까? (이 작업은 되돌릴 수 없습니다.)')) {
          if (data.profile) {
          ...
          alert('설정이 복원되었습니다. 메모 데이터는 기존 방식을 유지합니다.');
      ...
      if (profile?.notifications_enabled && currentTimeStr === profile.daily_reminder_time && lastNotificationDate.current !== todayStr) {
        ...
        if (Notification.permission === 'granted') {
          new Notification('Daily Harmony', {
            body: `${profile.name}님, 오늘 할 일이 ${todoCount}개 기다리고 있어요!`,
            icon: '/favicon.ico'
          });
          ...
          <div key={day.toString()} className={`relative min-h-[95px] md:min-h-[125px] p-2 border-r border-b cursor-pointer transition-all duration-200 ${!isSameMonth(day, monthStart) ? "bg-gray-50/50 text-gray-300" : "text-gray-700 bg-white"} ${isSelected ? "bg-indigo-50/50 ring-2 ring-inset ring-indigo-500/20 z-10" : "hover:bg-gray-50"}`} onClick={() => setSelectedDate(cloneDay)}>
            ...
            <div className="mt-1 md:mt-2 space-y-0.5 md:space-y-1 overflow-hidden">
               {dayMemos.slice(0, 3).map((m: Memo, idx: number) => (
                 <div key={idx} className="flex items-center space-x-1 animate-in fade-in slide-in-from-left-1">
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 animate-in fade-in duration-700">
      ...
      <div className="flex items-center justify-between mb-4 bg-white/60 backdrop-blur-sm px-4 py-2 rounded-2xl border border-gray-100/50 text-[10px] md:text-xs">
         <div className="flex items-center space-x-3">
           <div className={`flex items-center space-x-1.5 font-bold ${isSupabaseConfigured ? 'text-emerald-600' : 'text-amber-500'}`}>
             ...
             <span>{isSupabaseConfigured ? '실시간 클라우드 동기화 중' : '로컬 저장 모드 (서버 연결 필요)'}</span>
           </div>
           {isSyncing && <RefreshCw size={12} className="animate-spin text-indigo-400" />}
         </div>
         <div className="text-gray-400 font-medium">최근 업데이트: {format(new Date(), 'HH:mm:ss')}</div>
      </div>
          ...
          <div className="relative" ref={dataMenuRef}>
            ...
            {showDataMenu && (
              <div className="absolute right-0 mt-3 w-52 bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 py-2 z-[100] animate-in fade-in slide-in-from-top-2 origin-top-right">
                ...
                <button onClick={handleExportData} className="w-full flex items-center space-x-3 px-4 py-3.5 text-sm font-bold text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                  <Download size={18} className="shrink-0" /><span>데이터 백업하기</span>
                </button>
                <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center space-x-3 px-4 py-3.5 text-sm font-bold text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                  <Upload size={18} className="shrink-0" /><span>데이터 불러오기</span>
                </button>
                <input type="file" ref={fileInputRef} onChange={handleImportData} accept=".json" className="hidden" />
              </div>
            )}
          </div>
          <button onClick={() => setShowProfileModal(true)} className="flex items-center justify-center space-x-2 bg-white border border-gray-100 text-gray-700 px-4 md:px-6 py-2.5 md:py-3 rounded-2xl hover:bg-gray-50 transition-all shadow-sm font-bold text-sm md:text-base">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-30">
        <div className="lg:col-span-8">
          <div className="bg-white rounded-3xl p-3 md:p-6 shadow-2xl shadow-gray-200/40 border border-gray-50 overflow-x-auto relative">
            {loadingMemos && (
              <div className="absolute inset-0 bg-white/50 backdrop-blur-[2px] z-20 flex items-center justify-center rounded-3xl">
                <div className="flex flex-col items-center space-y-3">
                  <RefreshCw className="animate-spin text-indigo-600" size={32} />
                  <p className="text-sm font-black text-indigo-600">클라우드 동기화 중...</p>
                </div>
              </div>
            )}
            <div className="min-w-[320px]">
              <div className="grid grid-cols-7 mb-2 border-b pb-2">
                {["일", "월", "화", "수", "목", "금", "토"].map((day, i) => (
                  <div key={day} className={`text-center font-bold text-[10px] md:text-xs uppercase tracking-wider ${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : 'text-gray-400'}`}>{day}</div>
                ))}
              </div>
              {renderCells()}
            </div>
            ...
            <div className="mt-8 bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-6 md:p-8 border border-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 items-center">
                ...
                <div className="space-y-3">
                  {[{ label: '신체 지수', val: biorhythm?.physical, color: 'blue' }, { label: '감성 지수', val: biorhythm?.emotional, color: 'rose' }, { label: '지성 지수', val: biorhythm?.intellectual, color: 'emerald' }].map((item) => (
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center space-x-3 mt-4">
                ...
                <div className="flex items-center space-x-1.5 text-indigo-100">
                  <Moon size={14} /><span className="text-xs font-bold">음력 {currentDayInfo.lunar.getMonth()}.{currentDayInfo.lunar.getDay()}</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-7 border border-gray-50 overflow-hidden relative">
            ...
            {!apiKey ? (
              <div className="space-y-4 animate-in fade-in duration-500">
                <p className="text-xs text-gray-500 font-medium leading-relaxed">AI 운세 서비스를 위해 Gemini API 키를 연결해 주세요.</p>
                <div className="space-y-2">
                  <div className="relative">
                    <input type="password" value={tempKey} onChange={(e) => setTempKey(e.target.value)} placeholder="API 키를 입력하세요" className="w-full bg-gray-50 border-none focus:ring-2 focus:ring-indigo-500 rounded-xl py-3 pl-10 pr-4 text-xs font-medium" />
                    <Key size={14} className="absolute left-3.5 top-3.5 text-gray-300" />
                  </div>
                  <button onClick={handleSaveApiKey} className="w-full bg-indigo-600 text-white font-bold py-3 rounded-xl hover:bg-indigo-700 transition-all shadow-lg flex items-center justify-center space-x-2 text-xs">
                    <LinkIcon size={14} /><span>AI 서비스 연결하기</span>
                  </button>
                </div>
              </div>
            ) : !profile ? (
              <div className="py-4 text-center">
                 ...
                 <button onClick={() => setShowProfileModal(true)} className="mt-3 text-indigo-600 text-xs font-bold hover:underline">프로필 설정하러 가기</button>
              </div>
            ) : loadingFortune ? (
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-7 border border-gray-50">
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {([ [MemoType.TODO, ListTodo, '할일'], [MemoType.IDEA, Lightbulb, '아이디어'], [MemoType.APPOINTMENT, CalendarCheck, '약속'] ] as const).map(([type, Icon, label]) => (
                <button key={type} onClick={() => setSelectedType(type)} className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black transition-all ${selectedType === type ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>
            ...
            <div className="space-y-4">
              <div className="relative group">
                <input type="text" value={newMemo} onChange={(e) => setNewMemo(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddMemo()} placeholder="어떤 일을 기록할까요?" className="w-full bg-gray-50 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 rounded-2xl py-4 pl-5 pr-14 text-sm font-medium transition-all shadow-inner" />
                <button onClick={handleAddMemo} className="absolute right-2.5 top-2.5 p-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200"><Plus size={20} /></button>
              </div>
              <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100">
                <div className="flex items-center justify-between">
                  <button onClick={() => setShowReminderOptions(!showReminderOptions)} className="flex items-center space-x-2 text-xs font-bold text-gray-600 hover:text-indigo-600 transition-colors">
                  ...
                  <button onClick={() => setReminderEnabled(!reminderEnabled)} className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${reminderEnabled ? 'bg-indigo-600' : 'bg-gray-300'}`}><span className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${reminderEnabled ? 'translate-x-5' : 'translate-x-1'}`} /></button>
                </div>
                {reminderEnabled && showReminderOptions && (
                  <div className="mt-4 space-y-4 animate-in slide-in-from-top-2">
                    <div className="flex items-center space-x-3"><Clock size={14} className="text-gray-400" /><input type="time" value={reminderTime} onChange={(e) => setReminderTime(e.target.value)} className="bg-white border-none focus:ring-2 focus:ring-indigo-500 rounded-lg py-1.5 px-3 text-xs font-bold text-gray-700" /></div>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-7 border border-gray-50 min-h-[300px]">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-black text-gray-900">오늘의 기록</h3>
              {loadingMemos && <RefreshCw size={14} className="animate-spin text-indigo-400" />}
            </div>
            <div className="space-y-3">
              {currentDayMemos.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 opacity-40">
                   <ListTodo size={40} className="text-gray-200 mb-2" />
                   <p className="text-sm font-bold text-gray-300">오늘은 아직 기록이 없어요.</p>
                </div>
              ) : (
                currentDayMemos.map((memo) => (
                  <div key={memo.id} className="group flex flex-col bg-white border border-gray-50 p-4 rounded-2xl hover:border-indigo-100 transition-all animate-in fade-in slide-in-from-bottom-2">
                    {editingMemoId === memo.id ? (
                      <div className="space-y-4">
                        <input type="text" value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full bg-gray-50 border-none focus:ring-2 focus:ring-indigo-500 rounded-xl py-2.5 px-4 text-sm font-medium" />
                        <div className="flex space-x-2">
                          <button onClick={handleSaveEdit} className="flex-1 bg-indigo-600 text-white font-bold py-2.5 rounded-xl text-xs flex items-center justify-center space-x-1 shadow-lg shadow-indigo-100"><Check size={14} /> <span>저장</span></button>
                          <button onClick={() => setEditingMemoId(null)} className="flex-1 bg-gray-100 text-gray-500 font-bold py-2.5 rounded-xl text-xs flex items-center justify-center space-x-1"><X size={14} /> <span>취소</span></button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-4 flex-1 min-w-0">
                            <button onClick={() => handleToggleMemo(memo.id, memo.completed)} className="shrink-0 transition-transform active:scale-90">
                          ...
                          <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-all">
                            <button onClick={() => handleStartEdit(memo)} className="text-gray-300 hover:text-indigo-500 p-2"><Edit2 size={16} /></button>
                            <button onClick={() => handleDeleteMemo(memo.id)} className="text-gray-300 hover:text-rose-500 p-2"><Trash2 size={16} /></button>
                          </div>
                        </div>
                        {memo.reminder_time && (
                          <div className="mt-2 ml-10 flex items-center space-x-2">
                            <Bell size={10} className="text-indigo-400 shrink-0" />
                            <span className="text-[10px] font-bold text-indigo-400">{memo.reminder_time}</span>
                          </div>
                        )}
                      </>
                    )}
